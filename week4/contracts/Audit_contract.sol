...
// функцию назначания set_super_user сейтчас открытая и любой может ее вызвать и назначить себе права.
	function set_super_user(address _new_super_user) public {
//возможно сюда надо вставить проверку что только суперпользователь может назначать другим права
...
//функция публичная а значит любой может вызвать и удалить пользователя
	function remove_user(address _remove_user) public {
		require(users_map[msg.sender].created != 0);
//нет проверки кто может удалить пользователя, любой может подчистить базу
		delete(users_map[_remove_user]);
		bool shift = false;
// опасное расходование газа
...
// возможен неокднократный вызов функции до смены баланса
